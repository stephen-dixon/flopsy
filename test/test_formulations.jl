@testset "Formulations" begin
    ...
    @testset "SplitFormulation(LieSplit): returns SplitSolution" begin
        ...
        prob_lie = build_problem(
            model3, u03, tspan3, SplitFormulation(LieSplit()), config_lie)
    ...
    @testset "SplitFormulation saveat interpolates off-step times" begin
        ...
        sol_interp = solve_problem(
            build_problem(
                model_i, u0_i, tspan_i, SplitFormulation(LieSplit()), config_interp),
            config_interp
        )
    ...
    @testset "SplitFormulation(StrangSplit): higher accuracy than Lie" begin
        ...
        sol_strang = solve_problem(
            build_problem(
                model4, u04, tspan4, SplitFormulation(StrangSplit()), config_strang),
            config_strang
        )
    ...
    @testset "SplitFormulation: missing dt raises" begin
        ...
        config_no_dt = SolverConfig(
            ...
            reltol = 1e-7,
            show_progress = false,
            show_solver_stats = false            # dt not set
        )
        prob = build_problem(
            model5, u05, tspan5, SplitFormulation(LieSplit()), config_no_dt)
    ...
    @testset "solver_stats_dict: works on SplitSolution" begin
        ...
        sol_s = solve_problem(
            build_problem(model8, u08, tspan8,
...
@testset "check_mass_conservation" begin
    ...
    @testset "open system (Dirichlet BCs): surface flux accounts for loss" begin
        ...
        for ix in 1:nx
            ;
            dU0_o[1, ix] = 0.05 * sin(π * x[ix] / x[end]);
        end
