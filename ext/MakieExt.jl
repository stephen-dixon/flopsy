...
function _clamp_log(v::AbstractVector, min_val::Real, scale::Symbol)
...
function _tds_flux_from_df(df, flux_vars, T_col, surface, title; kwargs...)
    ...
    isempty(flux_cols) &&
        error("No flux columns found (surface=:$surface, flux_vars=$flux_vars)")
...
function _spatial_evolution_plot(x, t, fields, vars, times_req, group_fn,
    ...
    colors = [cmap[round(Int, 1 +
                                   (i-1)/(max(length(snap_idx)-1, 1)) * (length(cmap)-1))]
...
function _record_spatial_video_impl(result::SimulationResult, output_path;
    ...
    t, x, dx, fields = _extract_result_fields(result)
    return _do_record(
        x, t, fields, output_path, vars, fps, group_fn, title_fn, ylims; kwargs...)
end
...
function _do_record(
    ...
    for (ivar, vname) in enumerate(var_names)
        profile_obs = @lift begin
            ...
            haskey(grouped, vname) ? Float64.(grouped[vname]) :
            Float64.(fields[vname][$frame_idx, :])
        end
...
function _do_animate(x, t, fields, output_path, vars, fps, group_fn, title_fn,
    ...
    for (i, vname) in enumerate(var_names)
        profile_obs = @lift begin
            grouped = _apply_grouping(fields, $frame_idx, group_fn)
            raw = haskey(grouped, vname) ? Float64.(grouped[vname]) :
                  Float64.(fields[vname][$frame_idx, :])
