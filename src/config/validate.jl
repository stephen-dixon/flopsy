...
function validate(cfg::ProblemConfig)
    ...
    cfg.mesh.xmax > cfg.mesh.xmin || throw(ArgumentError("mesh.xmax must be greater than mesh.xmin"))
    ...
    if cfg.solver.formulation == :split
        cfg.solver.dt === nothing && throw(ArgumentError("solver.dt is required for split formulation"))
        cfg.solver.dt > 0 || throw(ArgumentError("solver.dt must be positive"))
    end

    if cfg.solver.saveat !== nothing
        issorted(cfg.solver.saveat) || throw(ArgumentError("solver.saveat must be sorted in ascending order"))
        for t in cfg.solver.saveat
            t0 <= t <= tend || throw(ArgumentError("solver.saveat time $t lies outside [$t0, $tend]"))
        end
    end

    for bc in cfg.boundary_conditions
        bc.side in (:left, :right) || throw(ArgumentError("boundary condition side must be :left or :right"))
        bc.kind == :dirichlet || throw(ArgumentError("Only dirichlet boundary conditions are currently supported"))
